...
int send_choice(connection conn)
{
	...
	while (resp == 0) {
		...
		if ((len > 0) && ((end - begin) < 60)) {
			...
			if (resp == 0) {
				printf
				    ("Chose another board number, that's already filled.\n");
				continue;
			}
		} else {
			buff[0] = 'O';
...
int main(int argc, char *argv[])
{
	...
	if (turn == 10 || turn == 20 || turn == 21) {
		if (turn == 10) {
			printf
			    ("You were out of time or send an empty anser.\n");
		} else if (turn == 20) {
			printf
			    ("Your opponent did not manage to anser on time or send an empty anser.\n");
		} else {
